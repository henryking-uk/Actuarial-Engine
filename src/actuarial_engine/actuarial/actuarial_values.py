...
class ActuarialValues:
    ...
    def whole_life_assurance_factor(self, age, sex):
        """Present value of a whole-life death benefit of £1."""

        Ax = 0

        for t in range(1, 101 - age):
            t__px = self.mortality.t_px(age, sex, t-1)
            qxt__ = self.mortality.table.qx(age + t - 1, sex)
    ...
    def whole_life_annuity_due_factor(self, age, sex):
        """Present value of a whole-life annuity-due of £1 per year."""

        ax = 0

        for t in range(0, 101 - age):
            t_px = self.mortality.t_px(age, sex, t)
            vt = self.interest.discount_factor(t)
            ax = ax + t_px * vt
    ...
    def term_assurance_factor(self, age, sex, n):
    ...
    def temporary_annuity_due_factor(self, age, sex, n):
    ...
    def increasing_whole_life_assurance_factor(self, age, sex, g):
